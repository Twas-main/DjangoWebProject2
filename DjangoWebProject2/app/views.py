...
def products(request, parameter):
  
    thing = Shop.objects.get(id=parameter)
    comments = Comment.objects.filter(post=parameter)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            comment_f.author = request.user
            comment_f.date = datetime.now()
            comment_f.post = Shop.objects.get(id=parameter)
            comment_f.save()

            return redirect("products", parameter=thing.id)
    else:
        form = CommentForm()
